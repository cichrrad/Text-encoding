...
int main() {
  ...
  std::priority_queue<std::pair<unsigned int, char>,
                      ...
                      std::greater<std::pair<unsigned int, char>>>
      pq;

  for (char c : s) {
    m[c] = m[c] + 1;
    std::cout << "Pushing \"" << c << "\" into m\n";
  }

  for (const auto &[k, v] : m) {
    std::cout << "[" << k << "] [" << v << "]\n";
    pq.push(std::pair<unsigned int, char>{v, k});
  }
  ...
  while (pq.size() != 1) {
    ...
    pq.pop();
    auto secondSmallest = pq.top();
    pq.pop();
    auto combined = std::pair<unsigned int, char>{
        (smallest.first + secondSmallest.first), '#'};
    pq.push(combined);
  }
